...
if "%RC%"=="0" (
  ...
  if errorlevel 1 (
    set "RC=1"
    echo.
    echo [ERROR] El proceso termino con codigo 0, pero no pude eliminar PRETLALPOWA.LOG: %LOG%
...
echo [OK] Para subirlo usa Publicar.cmd; la carpeta base no se convierte en repositorio Git.
...
:process_icon_pngs
if not exist "%BASE%\Iconos.cmd" (
  echo [ERROR] Falta Iconos.cmd para normalizar iconos PNG.
  >> "%LOG%" echo [ERROR] Falta Iconos.cmd para normalizar iconos PNG.
  exit /b 1
)
set "PRETLALPOWA_PARENT=Compilar"
call :run_live %ComSpec% /d /c call "%BASE%\Iconos.cmd" /nopause --root "%BASE%"
set "ICON_RC=%ERRORLEVEL%"
set "PRETLALPOWA_PARENT="
exit /b %ICON_RC%
...
:help
echo Uso:
echo   Compilar.cmd [clean] [debug^|release] [core] [run] [/noselftest] [/noiconpng] [/purge06] [/nmake^|/ninja] [/nopause]
echo.
echo Opciones:
...
exit /b 1
:icon_png_fail
echo [ERROR] Fallo la normalizacion previa de iconos PNG. Revisa PRETLALPOWA.LOG.
exit /b 1
:configure_fail
